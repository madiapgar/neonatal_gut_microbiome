...
**Reading in metadata file and fixing it**
```{r}
...
metadata %>% 
  ...
  mutate(age = as.factor(age)) -> metadata_red
```

**Reading in my unweighted Unifrac and Shannon files from Qiime2**
```{r}
...
**Putting the unweighted tables together so I can graph them**
```{r}
unweighted_unifrac %>% 
  left_join(metadata_red, by = 'sampleid') %>% 
  left_join(shannon, by = 'sampleid') -> unweighted_unifrac
```
